...
function App() {
  ...
  const contractAddress = "0x7aEEbF66BF122145fFdBe208d16c71DFbCD56d15"; // Paste the address recieved from Remix IDE here
          ...
          <div>
            <h2>Welcome, {name}</h2>  
              <div style={{ margin: '16px 0' }}>
                <input
                  type="text"
                  placeholder="Enter address to check registration"
                  value={checkAddress}
                  onChange={e => setCheckAddress(e.target.value)}
                  style={{ marginRight: '8px', padding: '4px' }}
                />
                <button onClick={checkIsRegistered}>Check Registration</button>
              </div>
            <p>Account: {account}</p>
